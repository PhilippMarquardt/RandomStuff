...
import React, { useState, useRef, DragEvent, MouseEvent as ReactMouseEvent, useEffect, useCallback } from 'react';
import type { 
  WorkflowTool, 
  CanvasTool, 
  DraggingToolDetails,
  DrawingConnectionDetails,
  Connection,
  EditingNodeDetails
} from '../types';
...
export default function DesignerPage() {
  const [availableTools] = useState<WorkflowTool[]>(initialAvailableTools);
  ...
  }, [selectedToolId, droppedToolsOnCanvas]);
              ...
              >
                ...
                <div className={`flex-1 bg-white p-3 rounded-b-lg border-x border-b ${colors.border}`}>
                  <p className="text-xs text-gray-500 truncate">
                    {(tool.properties?.description as string) || `Double-click to edit ${tool.name} properties`}
                  </p>
                </div>
            ...
            >
              ...
              <div>
                ...
                <textarea 
                  ...
                  defaultValue={(droppedToolsOnCanvas.find(t => t.instanceId === editingNode.instanceId)?.properties?.description as string) || ''}
                />
              </div>
