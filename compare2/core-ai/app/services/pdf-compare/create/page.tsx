      ...
      const boxesToExport = await Promise.all(
        annotationBoxes.map(async (box) => {
          ...
          if (originalSettings.positionIsNotGuaranteed) {
            // For not-guaranteed, we always use the single-call vision approach.
            // The UI no longer has a separate checkbox; its presence implies vision.
            cleanSettings.useVisionModel = true;
            
            // The model for the vision comparison call
            if (originalSettings.visionModel && originalSettings.visionModel !== 'None') {
                cleanSettings.visionModel = originalSettings.visionModel;
            }

            // The instruction for the comparison call
            cleanSettings.comparisonTaskDescription = originalSettings.comparisonTaskDescription || '';

            // Reference text for finding the page and focusing the model
            cleanSettings.referenceGuidingText = originalSettings.referenceGuidingText || '';

            if (!cleanSettings.referenceGuidingText && pdf.file) {
               try {
            ...
            // Logic for when position IS guaranteed
          ...
          // Image extraction is needed for the reference image if position is not guaranteed,
          // or for the direct comparison if position is guaranteed and using vision.
                                 ...
                                 <div className="pt-3 border-t border-gray-200">
                                   <div className="space-y-3">
                                     {selectedBoxes[0].settings.positionIsNotGuaranteed ? (
                                      // UI for the new single-call vision comparison
                                      <div className="space-y-3">
                                        <h5 className="text-sm font-medium text-gray-900 mb-2">Vision Comparison</h5>
                                        <div>
                                          ...
                                          <select
                                            ...
                                            onChange={(e) => handleUpdateAnnotationBox(selectedBoxes[0].id, {
                                                settings: { ...selectedBoxes[0].settings, visionModel: e.target.value }
                                            })}
                                        ...
                                        <div>
                                          ...
                                          <textarea
                                            ...
                                            onChange={(e) => handleUpdateAnnotationBox(selectedBoxes[0].id, {
                                                settings: { ...selectedBoxes[0].settings, comparisonTaskDescription: e.target.value }
                                            })}
                                            placeholder="e.g., Compare the two graphs and identify the month with the largest difference."
                                            className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                                            rows={3}
                                          />
                                        </div>
                                        <div>
                                          <label className="block text-xs font-medium text-gray-700 mb-1">
                                              Reference Text for Page Search
                                          </label>
                                          <textarea
                                            value={selectedBoxes[0].settings.referenceGuidingText || ''}
                                            onChange={(e) => handleUpdateAnnotationBox(selectedBoxes[0].id, {
                                                settings: { ...selectedBoxes[0].settings, referenceGuidingText: e.target.value }
                                            })}
                                            placeholder="Text to find the correct page and guide the model's focus."
                                            className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                                            rows={4}
                                          />
                                        </div>
