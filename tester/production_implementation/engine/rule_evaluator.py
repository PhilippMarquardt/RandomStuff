"""Rule evaluation engine - converts criteria dicts to Polars expressions."""
import json
import logging
...
class RuleEvaluator:
    ...
    def evaluate(
        ...
        precomputed_values: Optional[Dict[str, List[Any]]] = None
    ) -> pl.Expr:
        """
    ...
    def _evaluate_and(
        cls,
        subcriteria: List[Dict],
        perspective_id: Optional[int],
        precomputed_values: Optional[Dict]
    ) -> pl.Expr:
    ...
    def _parse_value(value: Any, operator: str) -> Any:
        ...
        if operator in ["In", "NotIn"]:
            if isinstance(value, str):
                # Handle formats like "('USD','EUR')" or "[4,8,9]"
        ...
        # Strip embedded quotes from string values
        if isinstance(value, str):
            return value.strip("'\"")
        return value
