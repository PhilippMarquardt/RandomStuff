...
export const usePDFAnnotation = ({ wordsData, pageNumber, onSelectBoxes }: UsePDFAnnotationOptions) => {
  ...
  const groupBoxes = useCallback((selectedBoxes: AnnotationBox[]) => {
    ...
    const groupedBox: AnnotationBox = {
      ...
      settings: { 
        ...
        visionModel: hasImageBoxes ? 'gemma3-vision' : 'None',
